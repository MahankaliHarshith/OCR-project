...
class ReceiptParser:
    ...
    def __init__(self, product_catalog: dict[str, str]):
        ...
        self._code_match_cache: dict[str, tuple[str, str, str | None]] = {}
        self._CODE_CACHE_MAX = 128
        self._cache_lock = threading.Lock()

        # ── Load learned OCR rules from training pipeline ──
        self._learned_char_rules: dict[str, str] = {}
        self._learned_reverse_rules: dict[str, str] = {}
        self._learned_code_corrections: dict[str, str] = {}
        self._load_learned_rules()

    def _load_learned_rules(self) -> None:
        """Load learned OCR substitution rules from training pipeline.

        The file ``training_data/learned_rules.json`` is generated by
        ``RealWorldTrainer.generate_learned_rules()`` and contains rules
        mined from real-world correction data.
        """
        from app.config import BASE_DIR

        rules_path = BASE_DIR / "training_data" / "learned_rules.json"
        ...
        try:
            ...
            self._learned_char_rules = data.get("ocr_char_rules", {})
            self._learned_reverse_rules = data.get("reverse_rules", {})
            self._learned_code_corrections = data.get("code_corrections", {})
            total = (
                len(self._learned_char_rules)
                + len(self._learned_reverse_rules)
                + len(self._learned_code_corrections)
            )
            if total:
                logger.info(
                    f"Loaded {total} learned OCR rules "
                    f"({len(self._learned_char_rules)} char, "
                    f"{len(self._learned_reverse_rules)} reverse, "
                    f"{len(self._learned_code_corrections)} code)"
                )
    ...
    def parse(self, ocr_results: list[dict], is_structured: bool = False) -> dict:
        ...
        if unparsed_lines:
            logger.debug(f"Unparsed lines: {[ln['text'] for ln in unparsed_lines]}")
    ...
    def _parse_line(self, text: str, confidence: float) -> dict | None:
        """
        ...
        if pfx_m:
            ...
            if cleaned_tail != numeric_tail:
                ...
                if len(collapsed_nums) >= 2 and len(collapsed_nums) < len(original_nums):
                    ...
                    logger.debug("    Split-number collapse SKIPPED (no change in token count)")
                else:
                    text = prefix + cleaned_tail
                    logger.debug(f"    Split-number collapse: {pfx_m.group(0)!r} → {text!r}")
    ...
    def _recover_stripped_qty(self, raw_text: str, cleaned_text: str) -> float:
        ...
        return stripped_num
    def _extract_qty_from_qt_marker(self, raw_text: str) -> float | None:
        """
    ...
    def _generate_ocr_variants(self, token: str) -> list[str]:
        ...
        if self._learned_char_rules:
            for i, ch in enumerate(token):
                if ch in self._learned_char_rules:
                    variant = token[:i] + self._learned_char_rules[ch] + token[i+1:]
                    v_upper = variant.upper()
                    if v_upper not in variants and v_upper:
                        variants.append(v_upper)

        if self._learned_reverse_rules:
            for i, ch in enumerate(upper):
                if ch in self._learned_reverse_rules:
                    variant = upper[:i] + self._learned_reverse_rules[ch] + upper[i+1:]
                    if variant not in variants and variant:
                        variants.append(variant)

        # Direct code correction (highest confidence — from repeated user corrections)
        if self._learned_code_corrections and upper in self._learned_code_corrections:
            corrected = self._learned_code_corrections[upper]
            if corrected not in variants:
                variants.insert(1, corrected)  # high priority — right after original
    ...
    def _try_fuzzy_code_extraction(self, text: str, confidence: float) -> dict | None:
        """
        ...
        for token in tokens:
            ...
            for variant in variants:
                ...
                if matches:
                    ...
                    if len(best) <= 3:
                        common = sum(1 for a, b in zip(variant, best, strict=False) if a == b)
    ...
    def _decode_qty_from_ocr_token(self, token: str) -> int | None:
        """
    ...
    def _decode_qty_from_dash_fragment(self, token: str) -> int | None:
        """
    ...
    def _identify_code_and_quantity(
        ...
        def _try_as_qty(s: str) -> float | None:
    ...
    def _map_product_code(self, code: str) -> tuple[str, str, str | None]:
        """
    ...
    def _get_product_info(self, code: str) -> dict | None:
    ...
    def _split_multi_product_line(self, text: str) -> list[str] | None:
        """
        ...
        sub_lines = []
        for i, (start, end, _code) in enumerate(positions):
            if i == 0:
                line_start = 0
            else:
    ...
    def _should_skip(self, text: str) -> bool:
        ...
        return bool(self._GRAND_TOTAL_KEYWORD_RE.search(text))
    ...
    def _is_total_line(self, text: str) -> bool:
        ...
        return not self._TOTAL_EXCLUDE_RE.search(text)
    ...
    def _cache_code_result(self, code: str, result: tuple[str, str, str | None]) -> None:
    ...
    def _get_cached_code(self, code: str) -> tuple[str, str, str | None] | None:
    ...
    def _extract_receipt_date(self, grouped_lines: list[dict]) -> str | None:
    ...
    def _extract_store_name(self, grouped_lines: list[dict]) -> str | None:
